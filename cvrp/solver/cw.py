...
class ClarkeWrightSolver(ISolver):
    ...
    def solve(self, problem: ProblemInstance) -> Optional[ProblemSolution]:
        ...
        for i, (id1, id2, s) in enumerate(savings):
            logger.debug(f"Iter {i}: {id1} -> {id2} ({s})")
            ...
            if not is_id1_assigned and not is_id2_assigned:
                ...
                logger.debug(f"Added new route {route.id}")
                continue
            ...
            if is_id1_assigned != is_id2_assigned:
                ...
                logger.debug(f"Added node {unassigned_id} to route {route.id}")
                continue
            ...
            if is_id1_assigned and is_id2_assigned:
                ...
                if route_id_1 == route_id_2:
                    logger.debug(f"Identical route {route_id_1}. Skipping")
                    continue
        ...
        for node in problem.nodes:
            ...
            route_counter += 1
            routes[route.id] = route
            logger.debug(f"Added single route {route.id} for node {node_id}")
